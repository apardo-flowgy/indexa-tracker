...
export function buildTrackerData(dataset) {
  ...
  // partial = same period slice (YTD or MTD) comparable across years
  // full    = entire year / month total (past years) or projected (current year)
  const _ytdPartialMap = new Map();
  const _mtdPartialMap = new Map();
  const _ytdFullMap    = new Map();   // whole-year totals
  const _mtdFullMap    = new Map();   // whole-month totals keyed by "year-month"
  for (const row of dataset.volumeRows) {
    const y = row.date.getFullYear(), m = row.date.getMonth(), d = row.date.getDate();
    if (m < _lm || (m === _lm && d <= _lday))
      _ytdPartialMap.set(y, (_ytdPartialMap.get(y) ?? 0) + row.inflowsDaily);
    if (m === _lm && d <= _lday)
      _mtdPartialMap.set(y, (_mtdPartialMap.get(y) ?? 0) + row.inflowsDaily);
    _ytdFullMap.set(y, (_ytdFullMap.get(y) ?? 0) + row.inflowsDaily);
    const mk = `${y}-${m}`;
    _mtdFullMap.set(mk, (_mtdFullMap.get(mk) ?? 0) + row.inflowsDaily);
  }
  const _isLeap   = (y) => y % 4 === 0 && (y % 100 !== 0 || y % 400 === 0);
  const _doy      = Math.round((_ld.getTime() - new Date(_ly, 0, 1).getTime()) / 86400e3) + 1;
  const annualProjection  = _doy > 0 ? (_ytdPartialMap.get(_ly) ?? 0) * (_isLeap(_ly) ? 366 : 365) / _doy : null;
  const _dimCurr          = new Date(_ly, _lm + 1, 0).getDate();
  const monthlyProjection = _lday > 0 ? (_mtdPartialMap.get(_ly) ?? 0) * _dimCurr / _lday : null;

  const ytdComparison = [..._ytdPartialMap.entries()]
    .filter(([y]) => y >= 2016).sort(([a], [b]) => a - b)
    .map(([year, partial]) => ({
      year,
      partial,
      full: year === _ly ? (annualProjection ?? partial) : (_ytdFullMap.get(year) ?? partial),
      isCurrent: year === _ly,
    }));

  const mtdComparison = [..._mtdPartialMap.entries()]
    .filter(([y]) => y >= 2016).sort(([a], [b]) => a - b)
    .map(([year, partial]) => ({
      year,
      partial,
      full: year === _ly ? (monthlyProjection ?? partial) : (_mtdFullMap.get(`${year}-${_lm}`) ?? partial),
      isCurrent: year === _ly,
    }));
