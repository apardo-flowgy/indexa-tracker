...
const I18N = {
  es: {
    ...
    aumSourcesInfo: "Descompone el crecimiento del patrimonio en tres fuentes. La revalorizacion NO es un residuo: se calcula de forma independiente marcando el AUM a mercado cada dia con los valores liquidativos oficiales de las carteras modelo de Indexa (la mezcla efectiva de carteras se estima con una regresion mensual contra las carteras 1 y 10). El residuo son los flujos no registrados como \"aportacion neta\": la propia Indexa reconoce en su pagina de estadisticas que esa cifra excluye aportaciones \"por fuera de Indexa\" a planes de pensiones o fondos de fondos hechas en otras plataformas (~1,5% de las aportaciones acumuladas, segun su nota oficial) — cifra muy cercana al residuo que estimamos aqui de forma independiente.",
    ...
    clientInflowInfo: "Modelo mensual que separa las aportaciones netas en dos componentes: dinero que llega con clientes nuevos y aportacion recurrente de los existentes. Como el numero de clientes y las altas mensuales crecen casi en paralelo (correlacion 0,96), una regresion libre no puede identificar el reparto; el ticket inicial se ancla al dato de la documentacion oficial de Indexa (~10.000 EUR) y se estima solo el componente recurrente. La serie de clientes se interpola entre observaciones, asi que la atribucion mes a mes es aproximada.",
    initialTicketLabel: "Ticket inicial por cliente nuevo (dato oficial)",
  ...
  en: {
    ...
    aumSourcesInfo: "Breaks AUM growth into three sources. Revaluation is NOT a residual: it's computed independently by marking AUM to market daily using Indexa's official model portfolio NAVs (the effective portfolio mix is estimated with a monthly regression against portfolios 1 and 10). The residual is flows not recorded as \"net contributions\": Indexa's own stats page discloses that this figure excludes contributions made \"outside of Indexa\" to pension plans or fund-of-funds on other platforms (~1.5% of accumulated contributions, per their official note) - a figure remarkably close to the residual we estimate here independently.",
    ...
    clientInflowInfo: "Monthly model that splits net contributions into two components: money arriving with new clients and recurring contributions from existing clients. Since total clients and monthly additions grow almost in parallel (correlation 0.96), a free regression cannot identify the split; the initial ticket is anchored to Indexa's official documentation (~10,000 EUR) and only the recurring component is estimated. The client series is interpolated between observations, so month-by-month attribution is approximate.",
    initialTicketLabel: "Initial ticket per new client (official figure)",
